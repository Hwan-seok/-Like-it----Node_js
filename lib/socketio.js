...
module.exports = (server, app) => {
    ...
    chat.on('connection', (socket) => { //네임스페이스 연결시 루프 동작
        ...
        socket.on("connection", (roomnum) => { //방 접속시에 현재 방번호 room에 저장
            ...
            msg.name = socket.request.user.name;
            ...
            const isRefresh = 'SELECT id FROM participants WHERE id=?'
            db.query(isRefresh, msg.sended, (err, result) => { //방에 
                if (result == undefined) return;
                else {
                    db.query(sql, [roomnum, msg.sended, msg.name, msg.nickname, msg.profile_image], (err, result) => {
                        room = roomnum;
